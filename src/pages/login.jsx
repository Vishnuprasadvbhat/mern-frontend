...
export const Login = () => {
  ...
  const onSubmitHandler = async (e)=>{
    try {
      ...
      if(state === "Sign Up"){
        const { data } =  await axios.post(backendUrl, '/api/auth/user/', {Name,email,password});
  ...
  return (
    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-400 to-indigo-700 text-white font-medium'>{state}</button>
      </form>

      {state === "Sign Up" ? (
        <p className='mt-3 text-gray-200 text-center text-xs-mt-4'>Already have an account? {''}
        <span onClick= {() =>setState('Login')}className='text-blue-400 cursor-pointer underline'>Login Here</span>
        </p>
        )

        : (
        <p className='mt-3 text-gray-200 text-center text-xs-mt-4'>Don't have an account? {''}
